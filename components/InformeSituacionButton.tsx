...
export default function InformeSituacionButton() {
  async function generarInforme() {
    ...
    try {
      const [respuestaViviendas, respuestaHabitaciones, respuestaInquilinos, respuestaFianzas, respuestaCobros] = await Promise.all([
        ...
        supabase.from("cobros").select("habitacion_id, inquilino_id, pendiente").in("estado", ["PENDIENTE", "PARCIAL", "DEUDA"]).gt("pendiente", 0),
      ]);
      const error = respuestaViviendas.error ?? respuestaHabitaciones.error ?? respuestaInquilinos.error ?? respuestaFianzas.error ?? respuestaCobros.error;
