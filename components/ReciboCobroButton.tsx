  ...
  async function generar() {
    ...
    if (estancia) {
      const { data: habitacionData, error: errorHabitacion } = await supabase
        .from("habitaciones")
        .select("id, codigo, vivienda_id")
        .eq("id", estancia.habitacion_id)
        .single();
      if (errorHabitacion) {
        alert(errorHabitacion.message);
        return;
      }
      ...
      const { data: viviendaData, error: errorVivienda } = await supabase
        ...
        .eq("id", habitacionHistorica.vivienda_id)
        .single();
      if (errorVivienda) {
        alert(errorVivienda.message);
        return;
      }
    ...
    if (!viviendaRecibo) {
      alert("No se ha encontrado la vivienda de este cobro.");
      return;
    }
    const { data: titularesVivienda, error: errorTitulares } = await supabase.from("vivienda_propietarios").select("propietario_id").eq("vivienda_id", viviendaRecibo.id);
