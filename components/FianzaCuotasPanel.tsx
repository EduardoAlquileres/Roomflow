...
export default function FianzaCuotasPanel({ fianzaId, estanciaId, importeTotal, entregado, fechaCobro, cuotas: iniciales }: Props) {
  ...
  const [correccionesPago, setCorreccionesPago] = useState<Record<string, string>>({});
  ...
  async function corregirPagoRegistrado(cuota: CuotaFianza) {
    const importeCorregido = numero(correccionesPago[cuota.id] ?? "");
    if (importeCorregido < 0 || importeCorregido > Number(cuota.importe) + 0.005) {
      alert(`El importe recibido debe estar entre 0,00 € y ${moneda.format(Number(cuota.importe))}.`);
      return;
    }
    if (!confirm(`Se corregirá el importe realmente recibido de esta cuota a ${moneda.format(importeCorregido)}. ¿Continuar?`)) return;
    setGuardando(true);
    try {
      const pagada = importeCorregido >= Number(cuota.importe) - 0.005;
      const fechaPago = importeCorregido > 0 ? cuota.fecha_pago ?? new Date().toISOString().slice(0, 10) : null;
      const { error: errorCuota } = await supabase.from("fianza_cuotas").update({
        importe_pagado: importeCorregido,
      ...
      alert(errorTexto(error, "No se pudo corregir el importe recibido."));
    } finally {
      setGuardando(false);
    }
  }
  ...
  async function guardarPlan() {
    ...
    setGuardando(true);
    try {
      const { error: errorFianza } = await supabase.from("fianzas").update({ importe: totalPactado, importe_entregado: entregadoSegunCuotas }).eq("id", fianzaId);
      if (errorFianza) throw errorFianza;
    ...
    {abierto && <div className="fixed inset-0 z-50 flex items-end bg-slate-950/40 p-0 sm:items-center sm:justify-center sm:p-6">
      <section className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-t-2xl bg-white p-5 shadow-xl sm:rounded-2xl">
        ...
        <div className="mt-5 space-y-3">{cuotas.map((cuota) => {
          const restante = Math.max(Number(cuota.importe) - Number(cuota.importe_pagado), 0);
          const exceso = Number(cuota.importe_pagado) > Number(cuota.importe) + 0.005;
          const pagada = !exceso && restante <= 0.005;
          return <article key={cuota.id} className="rounded-xl border border-slate-200 p-3">
            <div className="grid gap-3 sm:grid-cols-[1fr_130px_130px]">
              ...
              <div className="text-xs font-semibold text-slate-500">Estado<p className={`mt-2 text-sm ${exceso ? "text-red-700" : pagada ? "text-emerald-700" : "text-blue-700"}`}>{exceso ? `Revisar: registrado ${moneda.format(Number(cuota.importe_pagado))}` : pagada ? "Pagada" : `Pendiente ${moneda.format(restante)}`}</p></div>
            </div>
            <p className="mt-2 text-xs text-slate-500">Pactado: lo acordado para esta cuota. Recibido: solo el dinero que realmente se ha entregado.</p>
            ...
            {Number(cuota.importe_pagado) > 0 && <div className="mt-3 rounded-lg bg-slate-50 p-3"><p className="text-xs font-semibold text-slate-600">Corregir importe realmente recibido</p><div className="mt-2 flex gap-2"><input inputMode="decimal" placeholder={moneda.format(Number(cuota.importe_pagado))} value={correccionesPago[cuota.id] ?? ""} onChange={(event) => setCorreccionesPago((actual) => ({ ...actual, [cuota.id]: event.target.value }))} className="min-w-0 flex-1 rounded-lg border bg-white p-2 text-sm" /><button disabled={guardando || correccionesPago[cuota.id] === undefined || correccionesPago[cuota.id] === ""} onClick={() => corregirPagoRegistrado(cuota)} className="rounded-lg border border-amber-300 bg-white px-3 py-2 text-sm font-semibold text-amber-800 disabled:opacity-50">Corregir</button></div></div>}
