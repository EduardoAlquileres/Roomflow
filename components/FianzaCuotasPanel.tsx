...
type Props = { fianzaId: string; importeTotal: number; entregado: number; fechaCobro: string; cuotas: CuotaFianza[] };

export default function FianzaCuotasPanel({ fianzaId, importeTotal, entregado, fechaCobro, cuotas: iniciales }: Props) {
  ...
  function anadirCuota() {
    const falta = Number((importeTotal - sumaPlan).toFixed(2));
    if (falta <= 0) { alert("Primero reduce o reparte alguna cuota pendiente para dejar importe disponible."); return; }
    ...
    setCuotas((actual) => [...actual, { id: crypto.randomUUID(), fianza_id: fianzaId, numero: Math.max(...actual.map((cuota) => cuota.numero)) + 1, fecha_prevista: fecha.toISOString().slice(0, 10), importe: falta, importe_pagado: 0, fecha_pago: null, estado: "PENDIENTE", observaciones: null }]);
  }

  async function guardarPlan() {
    if (Math.abs(sumaPlan - importeTotal) > 0.01) { alert(`El plan debe sumar exactamente ${moneda.format(importeTotal)}. Falta repartir ${moneda.format(Math.max(importeTotal - sumaPlan, 0))}.`); return; }
    ...
    try {
      ...
      router.refresh();
    } catch (error) { alert(errorTexto(error, "No se pudo guardar el plan.")); } finally { setGuardando(false); }
  }
  ...
  async function registrarPago(cuota: CuotaFianza) {
    ...
    try {
      ...
      const { error: errorCuota } = await supabase.from("fianza_cuotas").update({ importe_pagado: nuevoPagado, fecha_pago: new Date().toISOString().slice(0, 10), estado: nuevoPagado >= Number(cuota.importe) - 0.005 ? "PAGADA" : "PENDIENTE" }).eq("id", cuota.id);
      ...
      router.refresh();
    } catch (error) { alert(errorTexto(error, "No se pudo registrar la entrega.")); } finally { setGuardando(false); }
  }
  ...
  async function crearPlanInicial() {
    ...
    try {
      ...
      for (let indice = 0; indice < (resto > 0 ? 2 : 0); indice += 1) { const fecha = new Date(inicio); fecha.setMonth(fecha.getMonth() + indice + 1); filas.push({ fianza_id: fianzaId, numero: filas.length + 1, fecha_prevista: fecha.toISOString().slice(0, 10), importe: indice === 1 ? Number((resto - cuota).toFixed(2)) : cuota, importe_pagado: 0, fecha_pago: null, estado: "PENDIENTE" }); }
      const { error } = await supabase.from("fianza_cuotas").insert(filas);
      if (error) throw error;
      router.refresh();
    } catch (error) { alert(errorTexto(error, "No se pudo crear el plan.")); } finally { setGuardando(false); }
  }
