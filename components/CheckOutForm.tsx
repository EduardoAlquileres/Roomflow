...
export default function CheckOutForm({ inquilinoId, habitacionId, fechaEntrada }: Props) {
  ...
  async function confirmarSalida() {
    ...
    if (!window.confirm("¿Confirmas el Check-Out? Saldrán todos los titulares de esta habitación, se cerrará la única fianza del alquiler y la habitación quedará libre.")) return;
  ...
  if (!mostrarFormulario) {
    return <button type="button" onClick={() => setMostrarFormulario(true)} className="rounded-lg border border-red-200 px-4 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-50">Realizar Check-Out</button>;
  }
    ...
    <div className="mt-6 rounded-lg border border-red-100 bg-red-50 p-4">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <label className="block text-sm font-medium text-gray-700">Fecha de salida
          <input type="date" min={fechaEntrada} value={fechaSalida} onChange={(event) => setFechaSalida(event.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 bg-white p-2" />
        </label>
        <label className="block text-sm font-medium text-gray-700">Observaciones de salida
          <input value={observaciones} onChange={(event) => setObservaciones(event.target.value)} placeholder="Opcional" className="mt-1 w-full rounded-lg border border-gray-300 bg-white p-2" />
        </label>
      </div>
      <fieldset className="mt-4">
        <legend className="text-sm font-semibold text-gray-800">Resolución de la fianza única</legend>
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          ...
          <label className={`cursor-pointer rounded-lg border p-3 ${!cumpleContrato ? "border-amber-400 bg-amber-50" : "border-gray-200 bg-white"}`}>
            ...
            <span className="mt-1 block text-xs text-gray-600">Compensación de gastos o incumplimientos; no es renta.</span>
          </label>
        </div>
        {!cumpleContrato && <label className="mt-3 block text-sm font-medium text-gray-700">Motivo de la retención<textarea value={motivoRetencion} onChange={(event) => setMotivoRetencion(event.target.value)} rows={3} placeholder="Ej.: desperfectos, salida sin aviso..." className="mt-1 w-full rounded-lg border border-gray-300 bg-white p-2" /></label>}
      </fieldset>
      {error && <p className="mt-3 text-sm text-red-700">{error}</p>}
      <div className="mt-4 flex justify-end gap-3">
        <button type="button" disabled={guardando} onClick={() => { setMostrarFormulario(false); setError(""); }} className="rounded-lg border bg-white px-4 py-2 text-sm font-medium disabled:opacity-50">Cancelar</button>
        <button type="button" disabled={guardando || !fechaSalida} onClick={confirmarSalida} className="rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{guardando ? "Finalizando..." : "Confirmar Check-Out"}</button>
      </div>
    </div>
  );
}
