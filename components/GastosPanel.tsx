...
import { filtrarGastos, filtrosGastosIniciales } from "@/lib/filtrosGastos";

type Props = { gastosIniciales: Gasto[]; viviendas: Vivienda[] };
type Formulario = { fecha: string; viviendaId: string; compartido: boolean; todas: boolean; seleccionadas: string[]; categoria: string; concepto: string; proveedor: string; importe: string; estado: "PAGADO" | "PENDIENTE"; observaciones: string; archivo: File | null };

const moneda = new Intl.NumberFormat("es-ES", { style: "currency", currency: "EUR" });
const inicial = (): Formulario => ({ fecha: new Date().toISOString().slice(0, 10), viviendaId: "", compartido: false, todas: true, seleccionadas: [], categoria: CATEGORIAS_GASTO[0], concepto: "", proveedor: "", importe: "", estado: "PAGADO", observaciones: "", archivo: null });
...
export default function GastosPanel({ gastosIniciales, viviendas }: Props) {
  ...
  const estadoOneDrive = parametros.get("onedrive");
  const detalleOneDrive = parametros.get("detalle");
  ...
  const [filtros, setFiltros] = useState(filtrosGastosIniciales);
  const gastosFiltrados = filtrarGastos(gastosIniciales, filtros);
  const anios = [...new Set(gastosIniciales.map((gasto) => gasto.fecha.slice(0, 4)))].sort().reverse();
  const activas = useMemo(() => viviendas.filter((vivienda) => vivienda.activa), [viviendas]);
  const destinosCompartidos = activas.filter((vivienda) => form.todas || form.seleccionadas.includes(vivienda.id));
  const total = gastosFiltrados.filter((gasto) => gasto.estado !== "ANULADO").reduce((suma, gasto) => suma + Number(gasto.importe), 0);
  ...
  function editar(gasto: Gasto) { setEditando(gasto); setForm({ fecha: gasto.fecha, viviendaId: gasto.vivienda_id, compartido: false, todas: true, seleccionadas: [], categoria: gasto.categoria, concepto: gasto.concepto.replace(" (prorrateado)", ""), proveedor: gasto.proveedor ?? "", importe: String(gasto.importe), estado: gasto.estado === "PENDIENTE" ? "PENDIENTE" : "PAGADO", observaciones: gasto.observaciones ?? "", archivo: null }); setError(""); setAbierto(true); }
  ...
  async function guardar(evento: FormEvent) {
    ...
    const destinos = form.compartido && !editando ? destinosCompartidos : viviendas.filter((vivienda) => vivienda.id === form.viviendaId);
    if (form.compartido && !editando && !destinos.length) { setError("Selecciona al menos una vivienda para repartir el gasto."); return; }
    if (!form.fecha || !form.concepto.trim() || !Number.isFinite(importe) || importe <= 0 || !destinos.length) { setError("Completa fecha, vivienda, concepto e importe."); return; }
