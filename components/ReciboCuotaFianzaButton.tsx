...
export default function ReciboCuotaFianzaButton({ fianzaId, cuota }: Props) {
  async function generar() {
    ...
    try {
      ...
      const { data: cuotasPlan, error: errorCuotasPlan } = await supabase
        .from("fianza_cuotas")
        .select("numero, fecha_prevista, importe, importe_pagado")
        .eq("fianza_id", fianzaId)
        .order("numero");
      if (errorCuotasPlan) throw errorCuotasPlan;
      ...
      const pagosPendientes = (cuotasPlan ?? [])
        .map((cuotaPlan) => ({
          ...cuotaPlan,
          pendiente: Math.max(Number(cuotaPlan.importe) - Number(cuotaPlan.importe_pagado), 0),
        }))
        .filter((cuotaPlan) => cuotaPlan.pendiente > 0.005);
      const pendienteTotalFianza = pagosPendientes.reduce((suma, cuotaPlan) => suma + cuotaPlan.pendiente, 0);
      const pagosPendientesHtml = pagosPendientes.length
