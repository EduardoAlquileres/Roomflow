...
export default function EditarCobroModal({ cobro, onCerrar, onGuardado }: {
  ...
  const total = Math.round((renta + suministros + Number(cobro.suplementos ?? 0)) * 100) / 100;
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4">
    <form onSubmit={guardar} role="dialog" aria-modal="true" aria-labelledby="editar-cobro-titulo" className="w-full max-w-lg space-y-4 rounded-xl bg-white p-6 shadow-2xl">
      ...
      {valido && <div className="rounded-lg bg-slate-50 p-3 text-sm">{Number(cobro.suplementos) > 0 && <p>Suplementos acordados: {euros(Number(cobro.suplementos))} (se editan en la habitación)</p>}<p>Total: {euros(total)}</p><p>Pagado: {euros(Number(cobro.pagado))}</p><p className="font-semibold">Pendiente: {euros(pendiente)}</p>{total < Number(cobro.pagado) && <p>Importe pagado de más: {euros(Number(cobro.pagado) - total)}. Este ajuste no realiza ninguna devolución.</p>}</div>}
